...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("forevercoin"))
        return false;
...
{
    // Convert forevercoin:// to forevercoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("forevercoin://"))
    {
        uri.replace(0, 12, "forevercoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "ForeverCoin.lnk";
}
...
{
    return GetAutostartDir() / "forevercoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("ForeverCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  forevercoin-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("ForeverCoin-Qt"));
